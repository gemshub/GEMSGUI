...
{
 SetFromString();
 string ret = templFrom->text().toUtf8().data();
 return ret;
}
...
{
 SetToString();
 string ret = templTo->text().toUtf8().data();
 return ret;
}
    ...
    if(rAll->isChecked()) // *abc, abc, invcase
    ...
    else //  ab*abc, invcase*a, bb*invcase, *bb, a*
     {
       if(chfirst->isChecked())
         ...
         ret =  lfirst->text().toUtf8().data();
       ret += "*";
       if(chlast->isChecked())
         ret += "invcase";
       else
         ret +=  llast->text().toUtf8().data();
      }
