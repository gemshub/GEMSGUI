...
GEM2MTWizard::GEM2MTWizard( const char* pkey, char flgs[32], int size[20],
                            ...
                            const vector<int>& vtk2, QWidget* parent  ):
    QDialog( parent ),  calcScript(acalcScript),
    outScript(aoutScript),  pageScript(0)
{
...
void GEM2MTWizard::resetVTKList()
{
    ...
    vector<pagesSetupData> scalarsList;
    vector<pagesSetupData> scalarsList2;
    vector<pagesSetupData> wnData;
...
{
  int nRow = -1;
  for(int ii=0; ii<scriptData.size(); ii++ )
  {
...
void GEM2MTWizard::setVTK( const vector<int>& vtk1, const vector<int>& vtk2  )
{

  scriptData.clear();

  for(int ii=0; ii<vtk1.size(); ii++ )
  {
    int cPage;
    int ndx;
...
void GEM2MTWizard::getVTK( vector<int>& vtk1, vector<int>& vtk2  )
{
  uint page_;
  int vt_1;
  vtk1.clear();
