        ...
        { // if we open graph, submodules and ... hide menu_record !!!
          ...
          else
          {
             ...
             if( pVisor->ProfileMode == MDD_DATABASE)
             {
               ...
               menubar->addAction(menuEJDB_Files->menuAction());
#ifdef oldDBusing
...
// Record
    ...
    // Record list
       ...
       connect( action_Buckup_EJDB, SIGNAL( triggered()), this, SLOT(CmBackupEJDB()));
       connect( actionRestore_EJDB, SIGNAL( triggered()), this, SLOT(CmRestoreEJDB()));
       //connect( actionList_from_EJDB, SIGNAL( triggered()), this, SLOT(CmListEJDB()));
       connect( actionExport_Json, SIGNAL( triggered()), this, SLOT(CmExportJson()));
       connect( actionImport_J_son, SIGNAL( triggered()), this, SLOT(CmImportJson()));
    ...
    // EJDB Database files
    connect( action_Selection_EJDB, SIGNAL( triggered()), this, SLOT(CmReOpenEJDB()));
    connect( action_Add_Link_EJDB, SIGNAL( triggered()), this, SLOT(CmAddOpenEJDB()));
    connect( action_New_Database_EJDB, SIGNAL( triggered()), this, SLOT(CmAddEJDBToList()));
    ...
    connect( actionIPAction, SIGNAL( triggered()), this, SLOT(CmSetIpActionAlgorithm()));
...
TCM_EV_COMMAND(CM_ADDFILETOLIST, CmAddEJDBToList);
TCM_EV_COMMAND(CM_ADDOPENFILE, CmAddOpenEJDB);
TCM_EV_COMMAND(CM_REOPENFILELIST, CmReOpenEJDB);
...
TCM_EV_COMMAND(CM_IMPORT3, CmImportJson);
TCM_EV_COMMAND(CM_EXPORT3, CmExportJson);
TCM_EV_COMMAND(CM_IMPORT4, CmBackupEJDB);
TCM_EV_COMMAND(CM_EXPORT4, CmRestoreEJDB);
...
void TVisorImp::CmSetKarpovAlgorithm()
{
    ...
    actionIPAction->setChecked(false);
...
void TVisorImp::CmSetIpActionAlgorithm()
{
    actionIPM3K_default->setChecked(false);
    actionIPM4K->setChecked(false);
    actionIPOPT->setChecked(false);
    actionIPNewton->setChecked(false);
    actionIPAction->setChecked(true);
 //   sactionStepwise->setChecked(false);
 //   sactionStepwise->setEnabled(false);
}
...
char TVisorImp::GetNumericalMethod()
{
    if (actionIPOPT->isChecked())
        ...
        return 'D';
    else if (actionIPAction->isChecked())
        return 'A';
    else
        return 'K';
}
        ...
        QDialog( win )
{
   ...
   const vector<string>& s = rt[iRt].GetOpenFiles();
   for( int ii=0; ii<s.size(); ii++ )
              pFileList->addItem(s[ii].c_str());

    pFileList->item(rt[iRt].GetFnum())->setSelected(true);
