...
InputSystemDialog::InputSystemDialog( QWidget* parent, const char* pkey,
    ...
    QDialog( parent ),
    ui(new Ui::InputSystemDialogData)
{
    size_t ii;
    ...
    ui->gridLayout_2->addWidget(pTname, 0, 1, 1, 1);

    pComment = new TLineEdit(this);
    pComment->setMaxLength(MAXFORMULA);
    ui->gridLayout_2->addWidget(pComment, 1, 1, 1, 1);

    recTable = new TRecipeTable(ui->groupBox_3, this);
    ui->gridLayout->addWidget(recTable, 1, 2, 2, 1);
    ...
    QObject::connect( ui->pPrint, SIGNAL(clicked()), this, SLOT(CmPrint()));
    QObject::connect( ui->pHelp, SIGNAL(clicked()), this, SLOT(help()));
    QObject::connect( ui->pCancel, SIGNAL(clicked()), this, SLOT(reject()));
    QObject::connect( ui->pOK, SIGNAL(clicked()), this, SLOT(accept()));
...
void InputSystemDialog::changePage( int nPage_ )
{
    ...
    if( nPage < wnData.size()-1 )
        ui->lDesc->setText( aObj[wnData[nPage].nObj]->GetDescription(0,0).c_str());
    else
        ui->lDesc->setText( "Other system recipe inputs (masses, volumes, T & P intervals)");
}
