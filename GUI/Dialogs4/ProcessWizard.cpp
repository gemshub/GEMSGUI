  ...
  case P_SEQUENT:
   ...
   case P_LIP:
     ...
     if( subtype == 0 || subtype == 1 )
     {
      ...
      if( lst.count() > 0 )
      {   string phname = lst[0].trimmed().toUtf8().data();
  ...
  switch(type)
  {
    case P_PVT:
      if( subtype == 2 )
      {
          ...
          {
              lst[ii] = lst[ii].trimmed();
              rowNames.push_back(lst[ii].toUtf8().data());
...
*/
  ...
  case P_INV_TITR:
        if( subtype == 1 )
        {
          ...
          {
             M_c1 = lst[ii].trimmed();
             rowNames.push_back(M_c1.toUtf8().data());
   ...
   case P_TITRSING:
               if( subtype == 0 )
               {
                 ...
                 ret = QString(
                         ...
                         "yp[0][1] =: %2 - ipH[0]; \n").arg(com, pH);
                pGraph->setValue( 2 );
                pageScript->setXname(pH.toUtf8().data());
                pageScript->setYname(com.toUtf8().data());
               }
       break;
   case P_REACTORS:
...
void ProcessWizard::setupPages()
{
  ...
  // clear old pages
   ...
   // init new pages
   for(int ii=0; ii<pgData.size();ii++)
    {
      nO = pgData[ii].nObj;
...
int  ProcessWizard::getNPoints( int col )
 {
     ...
     if( ( getType() == 'G'  && col == 6 ) || ( getType() == 'T' && col == 8 )) // workaround for checking min.addition of titrant
      ...
      {
          string str = tIters->horizontalHeaderItem( col )->text().toUtf8().data();
    ...
    if(  rt[RT_SYSEQ].GetStatus() != ONEF_) // read first record in DB
        {
            ...
            int Nr = rt[RT_SYSEQ].GetKeyList( ALLKEY, aRklist );
            if( Nr > 0 )
