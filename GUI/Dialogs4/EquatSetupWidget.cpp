...
{
  string res = textScript->toPlainText().toUtf8().data();
  return res;
}

vector<string> EquatSetup::getNames(string& xName, string& yName) const
{
    xName = xNam;
    yName = yNam;
...
void EquatSetup::setNames(const vector<string>& lst )
{
   namLines.clear();
...
void EquatSetup::CmAbscissa()
{
    ...
    string str = ndx->data(Qt::DisplayRole).toString().toUtf8().data();
...
void EquatSetup::CmAbscissaAdd()
{
    ...
    eqData.abscissaLines.push_back(
                getStringValue( nO, pLists[cPage]->currentRow(), str.c_str() ));
    /*if(nO<0)
      xNam = str;
    else
      xNam = aObj[nO].GetKeywd();
