...
QString TObjectModel::getHorizontalLabel(TObject *pObj, int iM) const
{
    if( !pObj )
        return "";
    if(pObj->GetMS() > 1 )
        return QString( "%1[%2]").arg(pObj->GetKeywd()).arg(iM);
    else
        return pObj->GetKeywd();
...
QVariant TObjectModel::data( const QModelIndex& index, int role ) const
{
    ...
    case  Qt::SizeHintRole:
        ...
        { // scroling for TextEdit fields
            ...
            return QSize( wdF(flds[ii].fType, flds[ii].npos+1, flds[ii].edit),
...
QVariant TObjectModel::headerData( int section, Qt::Orientation orientation, int role ) const 
{
    ...
    case Qt::DisplayRole:
        ...
        if( orientation == Qt::Horizontal )
        {
            ...
            {
                return getHorizontalLabel(aObj[nO].get(), iM);
            }
        }
        else
        {
    ...
    case Qt::SizeHintRole:
        ...
        else //if( orientation == Qt::Vertical )
        {
            if( flds[ii].label && iN == 0  && aObj[nO]->GetNS() == 1) // only line objects
                ...
                return QSize( wdF(ftString, 5, eNo ), htF(flds[ii].fType, flds[ii].maxN));
            else
