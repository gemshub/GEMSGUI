...
QString TObjectModel::getDescription( TObject* pObj, int N, int M) const
{
    ...
    string desc = pObj->GetDescription(N,M);
    QString s;
	 ...
	 return QVariant();	
        ...
        { case Qt::DisplayRole:
          case Qt::EditRole:
               if( nO == -1 )
                   return  QString("");
            return  QString::fromUtf8( visualizeEmpty( aObj[nO].GetStringEmpty( iN, iM ) ).c_str() );
          case Qt::ToolTipRole:
...
bool TObjectModel::setData( const QModelIndex &index, const QVariant &value, int role)
{
	...
	if( index.isValid() && ( role == Qt::EditRole ) )
	{
	  ...
	  {
        string txt = QVariant(value).toString().toUtf8().data();
     ...
     QTableView( parent ), flds(aFlds)
{
 ...
 {
   string txt = current.data(Qt::StatusTipRole).toString().toUtf8().data();
...
// scroling for TextEdit fields
 ...
 //if(  flds.count() > 3 )
 //     string(flds[2].pObj->GetKeywd(), 0, 4).find("mCIb") != string::npos
 //    )
 ...
 void TObjectTable::CmSDRef()
 {
  ...
  string str = index.data(Qt::EditRole).toString().toUtf8().data();
  try
  {
         bool patt = false;
         //save new SDrefs
         ...
         if( !strcmp( fld.pObj->GetKeywd(), "SDrefs") )
             sd_key = string( TSData::pm->GetKey() );
         ...
         if( patt )
         {
             ...
             {
                 // setValue();
                 if( sd_key.find_first_of("*?") == string::npos )  // pattern
  ...
  if(  fld.fType == ftCheckBox )
  {
    ...
    if( calc.exec() )
    {
      ...
      for(int nn=sel.N1; nn<=sel.N2; nn++)
      {
          ...
          model->setData(wIndex, QString(string(Vals, ii, 1).c_str()), Qt::EditRole);
        // fld.pObj->SetString( string(Vals, ii, 1).c_str(), nn, iM);
      }
    }
  }
 }
          ...
          Selection sel, bool transpose) throw(TError)
  {
     ...
     const int nLimit = (transpose) ? (sel.N1 + sel.M2-sel.M1) : sel.N2;
        ...
        for( int it = 0; it < rows.count() && rowNum <= nLimit; it++, rowNum++)
  	{
  	    ...
  	    {
          string value = (const char*)cells[ cellIt ].toUtf8().data();
          strip( value );
