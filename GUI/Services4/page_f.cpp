...
void CWinInfo::load(TConfig & cnf)
{
    ...
    while (!ss.empty())
    {
        aPageInfo.push_back( PageInfo(this, cnf, ss));
...
void CWinInfo::toDAT(ostream & os)
{
    // start signature
    os.write(SigBEG, 2);
...
void CWinInfo::fromDAT(istream & is)
{
    // start signature
    char sg[2];
    ...
    for (int ii = 0; ii < n; ii++)
        aPageInfo.push_back(  PageInfo(this, is));
...
void CWinInfo::toWinCFG(ostream & win_cfg)
{
...
void CWinInfo::fromWinCFG(istream & win_cfg)
{
    char name[100];
    win_cfg >> name;
    win_cfg >> init_width;
...
void PageInfo::load(TConfig & cnf)
{
    ...
    while (!obj.empty())
    {
        ...
        else
        {
            ...
            aFieldInfo.push_back(  FieldInfo( rO, ind, type, npos,
...
void PageInfo::toDAT(ostream & os)
{
    // begin signature
    os.write(PSigBEG, 2);
...
void PageInfo::fromDAT(istream & is)
{
    char sg[2];
    is.read(sg, sizeof sg);
    ...
    is.read((char *) &n, sizeof n);
    for (int ii = 0; ii < n; ii++)
        aFieldInfo.push_back( FieldInfo( is));
...
eFieldType PageInfo::GetType(const string & s)
{
