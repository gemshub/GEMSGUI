	...
	case o_wo_wx:
		  switch( aObj[o_wi_dcc].GetStringEmpty(iN)[0])
		  {
		    ...
		    case DC_SUR_CARRIER: /* sorbent */
		         ...
		         break;
		    default: // molality
                res =  QString::fromUtf8( visualizeEmpty(aObj[o_wd_ym].GetStringEmpty( iN, 0 )).c_str() );
...
bool TTreeModel::setData( const QModelIndex& index, const QVariant& value, int role)
{
	...
	if( index.isValid() && ( role == Qt::EditRole ) )
	{
           ...
           if( nO >= 0 )
	   {
		  ...
		  if( index.column()== 6 )
		  {	 
             lineFromIndex(index)->UGval = QVariant(value).toString().toUtf8()[0];
		  }	 
	 return true;
	} 
	return false;
