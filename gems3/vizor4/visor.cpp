...
//        SysGEMDir = getenv("HOME");
  ...
  //      UserGEMDir += "/.gems3/";
    ...
    QString dirExe = QCoreApplication::applicationDirPath () + QDir::separator();
...
TVisor::Setup()
{
    ...
    if (firstTimeStart)
    {
        ...
        if( !userGEM.mkdir(userGEMDir().c_str())
                || !userGEM.mkdir(userProfDir().c_str()) )
            throw TFatalError("GEMS Init", "Cannot create user GEMS directory");
...
void
TVisor::OpenModule(QWidget* parent, int i, int page, bool viewmode, bool select)
{
