//-------------------------------------------------------------------
// Id: gems/vizor/core/v_module.cpp  version 2.0.0    2001
//
...
void TDataBase::SetNewOpenFileList(TCStringArray& aFlKeywd)
{
    ...
    for(uint i=0; i<aFlKeywd.GetCount(); i++)
    {
        ...
        for(uint j=0; j<aFile.GetCount(); j++)
            if( strncmp(aFlKeywd[i].c_str(), aFile[j].GetKeywd(),
                        MAXFILEKEYWD)==0)
            {
                nF=j;
                break;
        ...
        if( nF >= 0 )
            ...
            Error( aFlKeywd[i].c_str(), "No such file name in this configuration!");
    }
...
void DataBaseList::check_num( int nR )
{
    ErrorIf( nR<0 || nR>(int)GetCount(),
