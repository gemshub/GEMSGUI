...
void TSupcrt::resid(double t, double *d)
{
    ...
    for (i=0; i<=8; i++) /* EQUIVALENCE (qr(3), qzr(1)), (qt(2), qzt(1)) */
    {
        qzr[i] = qr[i+2];
        qzt[i]=qt[i+1];
    }                  /* В as resalt: qzr[0]=qr[2], ... qzr[8]=qr[10];
                    ...
                    double *dV, double *delg, int epseqn)
{
    ...
    ideal(t);
    thmHGK(dV,t);
    /*    свойства пара будут содержаться в структуре FCTS
     *    в безразмерном виде (dimensionless); after
