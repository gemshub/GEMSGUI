//-------------------------------------------------------------------
// Id: gems/mods/submod/ms_muleq.cpp  version 2.0.0   2001
//
...
{
    // nessasary realloc data after new system   (before function )
    // setDefData();
    if( newRec == false )
        unpackData();
}
...
// tes.DCCW = (char *)aObj[ o_wi_dccw].Alloc( tes.L, 1, A_ );
 ...
 tes.DUL = new float[pm.L];
 memcpy( tes.DUL, pm.DUL, pm.L*sizeof(float) );
 tes.DLL = new float[pm.L];
 memcpy( tes.DLL, pm.DLL, pm.L*sizeof(float) );
 tes.Vol = new float[pm.L];
 memcpy( tes.Vol, pm.Vol, pm.L*sizeof(float) );
 tes.Pparc = new float[pm.L];
 memcpy( tes.Pparc, pm.Pparc, pm.L*sizeof(float) );
 tes.MM = new float[pm.L]; 
 memcpy( tes.MM, pm.MM, pm.L*sizeof(float) );
 ...
 tes.YOF = new float[pm.FI]; 
 memcpy( tes.YOF, pm.YOF, pm.FI*sizeof(float) ); 
 tes.lnGmM = new float[pm.L]; 
 memcpy( tes.lnGmM, pm.lnGmM, pm.L*sizeof(float) );
 tes.GEX = new float[pm.L]; 
 memcpy( tes.GEX, pm.GEX, pm.L*sizeof(float) ); 
 tes.FVOL = new float[pm.FI]; 
 memcpy( tes.FVOL, pm.FVOL, pm.FI*sizeof(float) );
 tes.FWGT = new float[pm.FI]; 
 memcpy( tes.FWGT, pm.FWGT, pm.FI*sizeof(float) ); 
 
 if( tes.L > 0 ) 
 {
   tes.Y_la = new float[pm.L]; 
   memcpy( tes.Y_la, pm.Y_la, pm.L*sizeof(float) ); 
   tes.Y_w = new float[pm.L]; 
   memcpy( tes.Y_w, pm.Y_w, pm.L*sizeof(float) );
   ...
   tes.Gamma = new float[pm.L]; 
   memcpy( tes.Gamma, pm.Gamma, pm.L*sizeof(float) ); 
   tes.lnGmf = new float[pm.L];
   memcpy( tes.lnGmf, pm.lnGmf, pm.L*sizeof(float) ); 
...
//    PMs and DMS may be realloced after load arrays LsMod and LsMdc
   ...
   tes.PUL = new float[pm.FIs];
   memcpy( tes.PUL, pm.PUL, pm.FIs*sizeof(float) ); 
   tes.PLL = new float[pm.FIs]; 
   memcpy( tes.PLL, pm.PLL, pm.FIs*sizeof(float) ); 
...
//     tes.RFSC = (char *)aObj[ o_wi_rfsc].Alloc( tes.FIs, 1, A_ ); 
 ...
 if( tes.LO > 1 ) 
 { 
   tes.Y_m = new float[pm.L];
   memcpy( tes.Y_m, pm.Y_m, pm.L*sizeof(float) ); 
 ...
 if( tes.E ) 
 {
    tes.EZ = new float[pm.L];
     memcpy( tes.EZ, pm.EZ, pm.L*sizeof(float) ); 
...
void TMulti::Test_Eq( int size, double *p1, double *p2, const char *key ) 
{
  ...
  for( int i=0; i<size; i++ ) 
    if( p1[i] != p2[i] ) 
    {
       sprintf(buf, "%s[%d] : %g %g", key, i, p1[i], p2[i]); 
       vfMessage( "Not equat double", buf.p); 
       break; 
    } 
...
void TMulti::Test_Eq( int size, float *p1, float *p2, const char *key ) 
{
  ...
  for( int i=0; i<size; i++ ) 
    ...
    { 
       sprintf(buf, "%s[%d] : %g %g", key, i, p1[i], p2[i]);
       vfMessage( "Not equat float", buf.p); 
       break; 
    } 
...
void TMulti::dyn__test(MULTI& tes) 
{
 ...
 Test_Eq( pm.FIs, tes.PMc, pm.PMc, "PMc" ); 
 Test_Eq( pm.FIs, tes.DMc, pm.DMc, "DMc" ); 
