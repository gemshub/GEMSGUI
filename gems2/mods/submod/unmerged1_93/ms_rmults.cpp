//-------------------------------------------------------------------
// Id: gems/mods/submod/ms_rmults.cpp  version 2.0.0   2001
//
...
void TRMults::LoadRmults( bool NewRec, bool changePhases )
{
    ...
    if( changePhases )
    {
        switch( vfQuestion3( window(), GetName(), "Change gaseous phase?",
                             "&Default", "&Select new" ))
        {
        case VF3_1:
            mu.PmvGas = S_ON;
...
// Select aqueous phase from all or make default
gstring TRMults::SelectAqPhase()
{
    ...
    do
    {
        i = vfChoice(window(), aPhaseList, "mu_aqkey Please, choose aqueous phase.");
    }
    while( i<0 );
    return  aPhaseList[i];
DEF_KEY:
