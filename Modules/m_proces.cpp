...
void TProcess::keyTest( const char *key )
{
    ...
    { // test project key
        string prfKey = rt[RT_PARAM].FldKey(0);
        int k = prfKey.length();
...
void TProcess::ods_link( int q)
{
    ...
    aObj[ o_pentxi ].SetPtr( &pe[q].Ntim ); /*i 3*/
    aObj[ o_penpvt ].SetPtr( &pe[q].NP );  /*i 3*/
...
//    pImp->MakeQuery();
    ...
    string xName = pep->xNames;
    string yName = pep->yNames;
     ...
     if(namesLines.size() > 0)
      {
         ...
         pep->lNam = (char (*)[MAXGRNAME])aObj[ o_pclnam ].Alloc( 1,
                    dimPclnam, MAXGRNAME);
         for(short ii=0; ii< min( (short)namesLines.size(),pep->dimXY[1]); ii++)
         {
...
//    {
        string pepstkey;
        if( !pep->Nst )
            rt[RT_SYSEQ].MakeKey( RT_PROCES, pepstkey, RT_PROCES, 0, RT_PROCES,1,
        ...
        strncpy(pep->stkey, pepstkey.c_str(), EQ_RKLEN+1);

        rt[RT_SYSEQ].SetKey(pep->stkey);
        if( rt[RT_SYSEQ].Find(pep->stkey))
        {
...
TProcess::RecordPlot( const char* /*key*/ )
{
    ...
    else
    {
      ...
      for( ii=0; ii<pep->dimXY[1]; ii++ )
          lnames.push_back( string( pep->lNam[ii+ndxy], 0,  MAXGRNAME ));
      for( ii=0; ii<pep->dimEF[1]; ii++ )
          lnames.push_back( string( pep->lNamE[ii], 0, MAXGRNAME ));
      gd_gr = new GraphWindow( this, plt, pep->name,
...
// We can only have one Plot dialog (modal one) so condition should be omitted!!
    ...
    plot = (TPlotLine *)
           aObj[ o_pcplline].Alloc( gr->lines.size(), sizeof(TPlotLine));
    for( ii=0; ii<gr->lines.size(); ii++ )
    {
        plot[ii] = gr->lines[ii];
        //  lNam and lNamE back
        if( ii < pep->dimXY[1] )
