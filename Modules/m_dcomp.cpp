...
{
    nQ = 2;
    aFldKeysHelp.push_back("Phase state code of Dependent Component { a g f p s l m c x y h }");
    ...
    else if( !stG && !stH && !stS )
    {
        ...
        if( fabs(S) > 1e-10 )
            if( fabs( (S_1 - S)/S ) >= DEF_REL_DEV )
            {
                G = H - T * ( S - foS );
                char Msgb[20];
                string s="W08DCrun: Inconsistent values of H0, S0 or G0 -> ";
                sprintf( Msgb, "%g", G );
                s += string(Msgb);
...
void TDComp::DCthermo( int q, int p )
{
    ...
    case CTPM_CPT:
        ...
        break;
    default:
        string msg = "E14DCrun: Invalid method code or data! ";
        msg += dcp->pstate;
        msg += "'.\nChange record?";
    ...
    case EMPC_:
        {
            ...
            if( db->Find( str.c_str() ) )
                Error( GetName(), " E19DCrun: This record alredy exists!");
            db->SetKey( str.c_str() );
            pVisor->OpenModule(window(), nRT,0,true);
            check_input(  db->UnpackKey() );
            RecBuild(  db->UnpackKey() );
...
void TDComp::CopyRecords( const char * prfName,
                          vector<int>& cnt,
         const elmWindowData& el_data, const dcSetupData& st_data,
   ...
   // get list of records
    ...
    str = db->PackKey();
     ...
     {  aICkey_new.push_back( stt );  // 30/11/2006
        ...
        { string sdkey = string( dcp->sdref[isd], 0, V_SD_RKLEN);
          strip(sdkey);
