...
void Set_SorpMod_Phase_coef();  // new
    void makeReacDCompList( const char *caption, vector<string>& aDclist,
                            short& nDC,  char (*SM)[DC_RKLEN], char * DCS, bool bNsuT);
    void makePhaseList( const char *caption, vector<string>& aPhlist );
    void makeICompList( const char *caption, vector<string>& aIclist );
    void DetNumbCatAn(vector<string>& aDclist);
    void LoadDCC();
...
public:
    ...
    void AssemblePhase( const char* key, const char* part, float *param,
           int file, bool useLst, const vector<string>& lst, int Npar = 4 );

    void CopyRecords( const char * prfName, vector<string>& aPHnoused,
                      vector<string>& aPHtmp, const elmWindowData& el_data,
                      const phSetupData& st_data,  vector<string>& SDlist);
    bool CompressRecord( int nDCused, const vector<int>& DCused, bool onlyIPX=false );

    void MakeSublatticeLists( const vector<string>& form_array  );
}
;
