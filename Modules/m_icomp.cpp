...
void
TIComp::GetElements( bool isotopes,
                     vector<string>& aIC,
                     vector<int>& aIndMT )
{
 vector<string> aIC1;
...
void TIComp::CopyElements( const char * prfName,
                      const elmWindowData& el_data,
                      const icSetupData& st_data )
{
  ...
  // delete the equvalent keys
   ...
   aICkey_new.clear();
    ...
    for(int j, i=0; i<el_data.ICrds.size(); i++ )
    {
       ...
       string stt = el_data.ICrds[i].substr(0,MAXICNAME+MAXSYMB);
