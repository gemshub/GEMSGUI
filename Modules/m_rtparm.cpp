...
bool TRTParm::check_input( const char *key, int Level )
{
    ...
    if( *srec == SRC_DCOMP )
    {
        ...
        aDC->ods_link(0);
        aDC->TCModule::TryRecInp( pkey.c_str(), tr, 0 );
    }
    else
    {
        TReacDC* aRC=(TReacDC *)(aMod[RT_REACDC]);
        aRC->ods_link(0); //Sveta 16/11/99
        aRC->TCModule::TryRecInp( pkey.c_str(), tr, 0 );
    }
    if( abs(Level) == 1 )
    {
        tra = rt[nRT].Rtime();
...
//    pImp->MakeQuery();
    ...
    if( rpp->expr )
     calcScript = rpp->expr;
    string xName = rpp->xNames;
    string yName = rpp->yNames;
    ...
    if(namesLines.size() > 0)
     {
        rpp->lNam = (char (*)[MAXGRNAME])aObj[ o_rtlnam ].Alloc( 1,
                         rpp->dimXY[1], MAXGRNAME);
        for(short ii=0; ii< min( (short)namesLines.size(),rpp->dimXY[1]); ii++)
        {
   ...
   if( rpp->Pplot != S_OFF )
    ...
    else
    {
      ...
      for( ii=0; ii<rpp->dimXY[1]; ii++ )
          lnames.push_back( string( rpp->lNam[ii], 0, MAXGRNAME ));
      for( ii=0; ii<rpp->dimEF[1]; ii++ )
          lnames.push_back( string( rpp->lNamE[ii], 0, MAXGRNAME ));
      gd_gr = new GraphWindow( this, plt, rpp->name,
...
// We can only have one Plot dialog (modal one) so condition should be omitted!!
    ...
    plot = (TPlotLine *)
    aObj[ o_rppline].Alloc( gr->lines.size(), sizeof(TPlotLine));
    for(int ii=0; ii<gr->lines.size(); ii++ )
    {
        plot[ii] = gr->lines[ii];
