...
void TDualTh::calc_eqstat()
{
   ...
   string  dtpsykey;
   rt[RT_SYSEQ].MakeKey( RT_DUALTH,  dtpsykey, RT_DUALTH, 0, RT_DUALTH,1,
         RT_DUALTH, 2, K_IMM, dtp->timep, K_IMM, dtp->Bnamep,
         K_IMM, dtp->Pp, K_IMM, dtp->TCp, K_IMM, dtp->NVp, K_END );
   strncpy( dtp->sykey, dtpsykey.c_str(), EQ_RKLEN );

   rt[RT_SYSEQ].SetKey(dtp->sykey);
// calc current SyStat
...
// {
  ...
  for( ii=0; ii< dtp->nQ; ii++ )
  {
     ...
     {
            string str = "Mole fractions calculation routine ";
 ...
 for( ii=0; ii < dtp->nQ; ii++)
 {
    ...
    for( j=0; j<dtp->nM; j++)
    {
       ...
       if( Chi < 1e-20 )
       {
          lnChi = log( 1e-20 );
          Chi = 1e-20;
          string str = "Zero or negative mole fraction chi";
