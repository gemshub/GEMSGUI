...
{
    const char * p_key;
    string name = string(ssp->name, 0, MAXFORMULA);
    string notes = string(ssp->notes, 0, MAXFORMULA);
    string EQkey = string(ssp->PhmKey, 0, EQ_RKLEN);
...
//            K_ANY, K_ANY, K_ANY, K_ANY, K_ANY, K_ANY, K_ANY, K_END);
        ...
        string skey = string(ssp->PhmKey, 0, EQ_RKLEN);
...
void TSysEq::InsertChanges( const vector<CompItem>& aIComp,
   const vector<CompItem>& aCompos, const vector<CompItem>& aPhase,
                            const vector<CompItem>&aDComp )
{
    int i;
        ...
        const char *oldName )
{
    ...
    db->SetKey(ALLKEY);
    db->SetFldKey(0, oldName );
    string str = db->PackKey();
    vector<string> arKey;

    int Nrec = db->GetKeyList( str.c_str(), arKey );
    if( Nrec < 1)
      return;

    for(int i=0; i<Nrec; i++ )
    {
        db->Get( arKey[i].c_str() );
        dyn_set();
        db->Del( arKey[i].c_str() );
        // changing record key
        db->SetFldKey(0,newName );
        str = db->PackKey();
        // change tPhEQ key (o_ssphst,PhmKey)
        string tPhkey = string( ssp->PhmKey, 0, EQ_RKLEN );
        if( tPhkey.find(oldName) != string::npos)
        {
