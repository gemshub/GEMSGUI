...
// get key of record
string TUnSpace::GetKeyofRecord( const char *oldKey, const char *strTitle,
                          int keyType )
{
    string str;
    if( oldKey == 0 )
...
// static objects
   ...
   aObj[ o_unpaflg ].SetPtr( &usp->Pa_f_pha ); /* a6 */
...
//  aMod[RT_UNSPACE].ModUpdate("GtDemo data sampling in progress...");
  ...
  if( TSyst::sm->GetSY()->Guns )
          fillValue( TSyst::sm->GetSY()->Guns, (double)0., (int)TRMults::sm->GetMU()->L );
  if( TSyst::sm->GetSY()->Vuns )
          fillValue( TSyst::sm->GetSY()->Vuns, (double)0., (int)TRMults::sm->GetMU()->L );
