        ...
        TCModule( nrt )
{
    ...
    setKeyEditField(8);
    /*aFldShot.push_back("PRname");
    aFldShot.push_back("GV");
    aFldShot.push_back("SYname");
    aFldShot.push_back("SYcsd");
    aFldShot.push_back("SYvol");
    aFldShot.push_back("SYp");
    aFldShot.push_back("SYt");
    aFldShot.push_back("SYvar");
    aFldShot.push_back("MTname");
    aFldShot.push_back("MTcod");
    **/
    mtp=&mt[0];
    set_def();
    start_title =
...
//    pImp->MakeQuery();
    ...
    if( mtp->gExpr )
      outScript = mtp->gExpr;
    string xName = mtp->xNames;
    string yName = mtp->yNames;
    ...
    if(namesLines.size() > 0)
    {
        mtp->lNam = (char (*)[MAXGRNAME])aObj[ o_mtlnam ].Alloc(
                      1, mtp->nYS, MAXGRNAME);
       for(long int ii=0; ii< min( static_cast<long>(namesLines.size()),mtp->nYS); ii++)
       {
...
TGEM2MT::RecCalc( const char * key )
{
 try
 {
   ...
   if( mtp->PsVTK != S_OFF )
   {
     prefixVTK = db->FldKey(8);
     prefixVTK += "_";
     prefixVTK += db->FldKey(9);
     prefixVTK += "_";
     nameVTK = db->PackKey();
     replace(nameVTK.begin(), nameVTK.end(),':', '_');
   ...
   else
   {
     ...
     //              "Please, edit the GEMS4K filelist name", "*.lst" ) == false )
     //        return;
      }
...
void TGEM2MT::RecordPrint( const char* key )
{
	...
	if( res == VF3_1 )
	{
        string filename = "GEM2MT-task.dat";
...
void TGEM2MT::InsertChanges( const vector<CompItem>& aIComp,
      const vector<CompItem>& aPhase,  const vector<CompItem>&aDComp )
{
