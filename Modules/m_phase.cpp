...
{
    nQ = 1;
    aFldKeysHelp.push_back("Code of phase state { a g f p l m h s d x }");
    ...
    /*aFldShot.Add("PHcod");
    aFldShot.Add("PHgroup");
    aFldShot.Add("PHname");
    aFldShot.Add("PHclass");
    aFldShot.Add("PHcom");*/
...
TPhase::RecBuild( const char *key, int mode  )
{
    ...
    vector<string> aPhlist;
    vector<string> aDcSkrl;
    //string str;
    TProfil *aPa=(TProfil *)(aMod[RT_PARAM]);
...
void TPhase::CopyRecords( const char * prfName, vector<string>& aPHnoused,
                          vector<string>& aPHtmp, const elmWindowData& el_data,
                          const phSetupData& st_data,  vector<string>& SDlist )
{
    //TCIntArray anR;
    vector<int> aDCused;
    vector<string> aPHkey;
    aPHnoused.clear();
  ...
  // get list of records
    ...
    for(size_t ii=0; ii<aPHkey.size(); ii++ )
    {
        size_t jj;
...
//       continue;
    ...
    string stt = aPHkey[ii].substr(0,MAXSYMB+MAXPHSYMB+MAXPHNAME+MAXSYMB);
...
// Test existence of DComp/ReacDC records
     ...
     for( i=0, cnt=0; i<php->nDC; i++ )
     {
        ...
        // only 3 fields
        string key = string( php->SM[i], 0, DC_RKLEN);
...
//             case SM_OTHER: // = 'O',	// other built-in phase-specific models of non-ideal solutions (selected by phase name)
   ...
   {  aICkey_new.push_back( stt );  // 30/11/2006
      ...
      { string sdkey = string( php->sdref[isd], 0, V_SD_RKLEN);
        strip(sdkey);
...
bool TPhase::CompressRecord( int nDCused, const vector<int>& DCused, bool onlyIPX )
{
 int ii, jj, DCndx;
 int nDCnew;
 int ncpNnew;
