...
void TSyst::setDefData()
{
    ...
    if( sy.PbIC != S_OFF && sy.PdelB != S_OFF )
        memset( sy.delB, 0, mup->N*sizeof(double) );
    if( sy.PbDC != S_OFF )
    {
...
//             sy.MaSdj[i] = FLOAT_EMPTY;
    ...
    if( sy.PYOF != S_OFF )
        memset( sy.YOF, 0, mup->Fi*sizeof(double) );
    if( sy.PParc != S_OFF )
        memset( sy.Pparc, 0, mup->L*sizeof(double) );
    if( sy.PGEX != S_OFF )
        memset( sy.GEX, 0, mup->L*sizeof(double) );
    if( sy.PlnGf != S_OFF )
        memset( sy.lnGmf, 0, mup->L*sizeof(double) );
...
void TSyst::packData( const vector<int>& PHon, const vector<int>& PHoff,
                      const vector<int>& DCon, const vector<int>& DCoff )
{
    short i, i1, i2, i3, i4, i5, j;
    int iOff;
    TProfil *aPa=(TProfil *)(aMod[RT_PARAM]);
