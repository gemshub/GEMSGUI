...
void TSyst::systbc_calc( int mode )
{
    ...
    {        
      ...
      if( ProvidedPhm )
      {
        ...
        skey = string(aSE->ssp->PhmKey, 0, EQ_RKLEN);
...
//           syPPHk = sy.PPHk;
           ...
           try
           {
                  if( !rt[RT_SYSEQ].Find( skey.c_str() ) )    // Here call dialog for selection of the SysEq key
                  {
...
//                  sy.PbPH = syPbPH;
//                  sy.PPHk = syPPHk;
                  rt[RT_SYSEQ].Get( skey.c_str() );  // Read record
                  aSE->dyn_set(1);
...
// SD 14/03/02
    ...
    for( i=0; i<N; i++ )
    {
        ...
        {
            aICkeys.push_back(string(mup->SB[i], 0, MAXICNAME));
            aICnums.push_back(i);
            sy.Icl[i] = S_REM;
        }
        else
    ...
    if( aICkeys.size() > 0 )
    {
      if( vfExcludeFillEdit( window(), rt[RT_SYSEQ].PackKey(),
         ...
         for( int ii=0; ii<aICnums.size(); ii++ )
         {
           i = aICnums[ii];
           if( sel[ii]== true ) //fill
           {
