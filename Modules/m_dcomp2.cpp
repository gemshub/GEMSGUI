//-------------------------------------------------------------------
// $Id: m_dcomp2.cpp 1409 2009-08-21 15:34:43Z gems $
//
...
//--------------------------------------------------------------------------
// calculate temperature correction to thermodynamic properties
void TDComp::calc_tpcv( int q, int p, int CE, int CV )
{
    double T, TC, Tst, T_Tst, Ts2, TT, dT, T2, T3, T4, T05,
    ...
    {  // Cp(t)  current temperature only
        ...
        if( IsFloatEmpty( a1 ) || approximatelyZero(a1, (float)1e-14) )
            ac[i] = 0.0;
        else
            ac[i] = a1;
     }
...
NEXT:
    ...
    // Holland-Powell phases with Landau transition (with Murnaghan EoS)
    ...
    if( CE == CTM_CHP )  // Corrected 02.04.2011 (TW)
    {
        ...
        if( !IsFloatEmpty( Tcr0 ) && !IsFloatEmpty( Smax ))
        {
            if ( Smax <= 0)
                Smax = 1.0e-20;
            ...
            v_bis = Vmax *pow(Q298,2.) * ( 1.+a0*(T-Tst) - 20.*a0*( sqrt(T) - sqrt(Tst) ));
            ivdp = (v_bis*kT/3.) * ( pow((1.+4.*pp/kT),0.75) - 1. ) * (1000.);
            // idvdtdp = Vmax*pp*(a0-10.*a0/sqrt(T)) * (1000.);
            idvdtdp = ( ((Vmax*pow(Q298,2.))*kT/3.)*(a0-10.*a0*pow(T,-0.5)) * (pow((1.+4.*pp/kT),0.75)-1.)
                            - ((v_bis)*k298/3)*(1.5e-4) * (pow((1.+4.*pp/kT),0.75)-1.)
                            + (v_bis)*pp*(1.5e-4)/(1.-(1.5e-4)*(T-Tst)) * pow((1.+4.*pp/kT),-0.25) ) * (1000.);
            smq = Smax * Tcr0 *( pow(Q298,2.) - pow(Q298,6.)/3. );

            // increment thermodynamic properties
            aW.twp->G += ( smq - T*Smax*pow(Q298,2.) + ivdp + Smax*((T-Tcr)*pow(Qq,2.) + Tcr*pow(Qq,6.)/3.) );
            aW.twp->S += ( Smax * ( pow(Q298,2.) - pow(Qq,2.) ) - idvdtdp );
            aW.twp->H += ( smq - Smax*Tcr*( pow(Qq,2.) - pow(Qq,6.)/3.) + ivdp - T*idvdtdp );
            aW.twp->V = ( v_bis + 2.*Smax*(T-Tcr)*Qq*dQq - Smax*pow(Qq,2.)*Vmax/Smax + Tcr*pow(Qq,2.)*dQq
...
//-----------------------------------------------------------------------------
// Calculation of V*dP correction to standard properties of solids (minerals)
...
void TDComp::calc_voldp( int q, int /*p*/, int /*CE*/, int CV )
{
    ...
    // Calculation of molar volume with Murnaghan EoS (Holland and Powell, 1998)
    else if( CV == CPM_CEH )
    {
        // reading and checking coeffs
        k0 = (double)dc[q].Comp;  // bulk modulus k in kbar at 298 K
        // k0 *= 1000.;
    	if( IsFloatEmpty( dc[q].Comp ))
                k0 = 0.;
        a0 = (double)dc[q].Expa; // a0 parameter in 1/K
    	if( IsFloatEmpty( dc[q].Expa ))
                a0 = 0.;
    	if(( fabs( P_Pst ) > PRESSURE_PREC || fabs( T_Tst ) > TEMPER_PREC )
                        && fabs(Vst) > 1e-9 && fabs(k0) > 1e-9 && fabs(a0) > 1e-9 )
    		// No zero molar volume
    	{
                PP = P/1000.;  // P is used in kbar in HP98
                aW.twp->Alp = a0 * (1. - 10./pow(T,0.5) );  // expansion at T
                kap = k0 * ( 1. - (1.5e-4)*(T-Tst) );
                aW.twp->Bet = 1./PP * (1. - pow( (1.- 4.*PP/(kap + 4.*PP )), 0.25 ));  // compressibility at T (to check)
                Vt = Vst * (1.+ a0*(T-Tst)-20.*a0*(pow(T,0.5)-pow(Tst,0.5)) );

                // increment thermodynamic properties
                dg = ( (1./3.)*Vt*kap*(pow((1.+4.*PP/kap),0.75 )- 1.) * (1000.) );
                ds = ( - ( (Vst*kap/3.)*(a0-10.*a0*pow(T,-0.5)) * (pow((1.+4.*PP/kap),0.75)-1.)
    ...
    {  // Vm = f(T,P) equations
        ...
        for( i=0; i<5; i++ )
        {
         ...
         aE = vc[0] + vc[1]*2.*T_Tst + vc[2]*3.*(T_Tst*T_Tst);
         aC = - vc[3] - vc[4]*2.*P;
         ...
         {  // can be calculated
             ...
             aW.twp->G += ( VP );
             aW.twp->H += ( VP );
             aW.twp->V += ( vc[0]*VT + vc[1]*VT*T_Tst + vc[2]*VT*Ts2 + vc[3]*VP + vc[4]*VP*P_Pst );
             aW.twp->S += ( - vc[0]*VP - vc[1]*VP*2.*T_Tst - vc[2]*VP*3*Ts2 );
             aW.twp->G += ( vc[0]*T_Tst*VP + vc[1]*VP*Ts2 + vc[2]*VP*Ts2*T_Tst + vc[3]*VP*P_Pst / 2.
                          + vc[4]*VP*(P_Pst*P_Pst) / 3. );
             aW.twp->H += ( - vc[0]*Tst*VP - vc[1]*VP*(T + Tst)* T_Tst + vc[2]*VP*Ts2*(2.*T + Tst)
          ...
          double &dG, double &dH, double &dS )
{
   double vt, /*vpt,*/ a1, a2, a3, /*a4, a5,*/ kt00, kt0, dkdt, kp, kpp, vstart,
          ...
          vPplus, vPminus, vTplus, vTminus, kt0Tplus, kt0Tminus, kppTplus=0., kppTminus=0.,
...
// calculation of temperature and pressure corrections for aqueous species
void
...
// Shock et al.,1991) and it (dgdP, dgdT, d2gdT2 for TdegC, Pbars  for algoritm:
// geqn = 1 ...... use Tanger-Helgeson (1988) equations
// geqn = 2 ...... use Shock et al. (1991) equations without the f(P,T) difference function
// geqn = 3 ...... use Shock et al. (1991) equations with the f(P,T) difference function
