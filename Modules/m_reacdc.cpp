        ...
        TCModule( nrt )
{
    ...
    aFldKeysHelp.push_back("Phase state code of new Dependent Component { a g f p s l m c x y h }");
...
AGAIN_MOD:
    ...
    if( Nc1>0 || Nr1>0 )
    {
        ...
        for( i=0; i<rcp->nDC; i++ )
        {
          ...
          {
              key_dr  = string(1, rcp->rDC[i]);
              key_dr += ' ';
              key_dr += string( rcp->DCk[i], 0, DC_RKLEN-MAXSYMB );
              aDclist_old.push_back( key_dr.c_str() );
          }
          /*
          string key_dr = string( rcp->DCk[i], 0, DC_RKLEN-MAXSYMB ); // SD 18/11/2008
...
AGAINRC:
   ...
   aDclist = vfRDMultiKeysSet( window(),
    ...
    /*================================*/
    rcp->nDC =(short)( aDclist.size()/*+aRclist.GetCount()*/+Nn1+Nf1);
    // ???? 28/02/02 Sveta
...
CALCULATE_DELTA_R:
    ...
    default:
        {  /* Invalid code method of calculation?*/
            string msg = "W14RErun: Invalid CM method flag!";
            msg += dckey;
            msg += "'.\n Change the record?";
...
//        G = G4;
    ...
    int fnum = -1;
    bool yesToAll=false;
    rt[RT_DCOMP].SaveRecordQuestion(dcn.c_str(), fnum, yesToAll );
    /* A reminder */
...
void TReacDC::TryRecInp( const char *key_, time_t& time_s, int q )
{
    ...
    dbKey.SetKey(key_);
    dbKey.SetFldKey(3,"*");
    string str_key= dbKey.UnpackKey();
    ...
    case MANY_:    // Get Key list
        db->GetKeyList( str_key.c_str(), aDclist );
        db->Get(aDclist[0].c_str());
    case ONEF_:
        dyn_set(q);
        time_s = db->Rtime();
        return;
    ...
    case EMPC_:
        {
            ...
            ErrorIf( !db->Find( str.c_str()) , GetName(), " W20RErun: This record alredy exists!");
            db->SetKey( str.c_str() );
            pVisor->OpenModule(window(), nRT,0,true);
            string str1 = db->UnpackKey();
...
void TReacDC::CopyRecords( const char * prfName,
         vector<int>& cnt,  const elmWindowData& el_data,
         const rdSetupData& st_data,  vector<string>& SDlist)
{
    vector<string> aDCkey;
    ...
    for(int ii=0; ii<aDCkey.size(); ii++ )
    {
     ...
     string stt = aDCkey[ii].substr(0,MAXSYMB+MAXDRGROUP+MAXDCNAME);
