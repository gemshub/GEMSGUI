...
void TProfil::TestChangeProfile()
{
    ...
    // compare ICOMP list
    vector<CompItem> aIComp;  // list of IC changes
    ICcompare( aIComp);
    // compare COMPOS list
    vector<CompItem> aCompos;  // list of Compos changes
    COMPcompare( aCompos );
    // compare PHASE list
    vector<CompItem> aPhase;  // list of Phase changes
    vector<CompItem> aDComp;  // list of DComp changes
    ...
    if( aIComp.size()>=1 || aPhase.size()>=1 || aDComp.size()>=1 )
    {
      aList.clear();
       ...
       rt[RT_GEM2MT].GetKeyList( pkey.c_str(), aList );
...
AGAIN:
    ...
    {
       sprintf( tbuf, "Project: %s; Systems: %d; Errors: %d",
                ProfName.c_str(), i, nbad );
       iRet =  pVisor->Message( 0,  "Re-calculating and saving all equilibria",
                                tbuf, i, aList.size() );
      if( iRet )
        break;
...
void TProfil::ShowDBWindow( const char *objName, int nLine )
{
    ...
    case 'C': // Compod : CC_v__
        if( strncmp(objName, aObj[o_musa].GetKeywd(), MAXKEYWD)==0)
            s = string( mup->SA[nLine], 0, BC_RKLEN );
        else break;
    ...
    case 'D': // Phase : DC_v__ or DC_v2 or DCnam or DCnam2
        if( strncmp(objName, aObj[o_musm].GetKeywd(), MAXKEYWD)==0 ||
            ...
            s = string( mup->SM[nLine], 0, DC_RKLEN );
...
"Copying Kernel database records to Modelling Project. Please, wait...", 10  );
    ...
    {  // List of Phases or Compos with some species discarded
        ...
        string filename = pVisor->userGEMDir();
...
void TProfil::systbcInput( QWidget* par, const char * p_key )
{
    ...
    // define window
    vector<windowSetupData> wnData;
    // define table
    vector<tableSetupData> tbData;
    // define scalars
...
//    if(  fabs( syp->Mbel ) > 0  )
//     tbData.push_back(  tableSetupData( wnData.GetCount()-1, o_symass,
...
string TProfil::PhNameforDC( int xdc, bool system )
{
...
string TProfil::PhNameforDC( int xdc, int& xph, bool system )
{
...
vector<string> TProfil::DCNamesforPh( const char *PhName, bool system )
{
  ...
  { for( k=0; k<mup->Fi; k++ )
    ...
    for( j= DCx; j<DCx+mup->Ll[k];j++ )
      {
        dcstr = string( mup->SM[j]+MAXSYMB+MAXDRGROUP, 0, MAXDCNAME );
        strip(dcstr);
  ...
  { for( k=0; k<pmp->FI; k++ )
    ...
    {   dcstr =  string( pmp->SM[j], 0, MAXDCNAME );
        strip(dcstr);
        DCnames.push_back(dcstr);
    }
  }
...
void TProfil::DCNamesforPh( int xph, bool system, vector<int>& xdc, vector<string>& dcnames)
{
    int k, j, DCx = 0;
    RMULTS* mup = rmults->GetMU();
    MULTI*  pmp = multi->GetPM();
    ...
    { for( k=0; k<xph; k++ )
      ...
      for( j= DCx; j<DCx+mup->Ll[xph];j++ )
        {
          xdc.push_back(j);
          dcnames.push_back( string( mup->SM[j], 0, DC_RKLEN ));
        }
    }
    else
    {
      ...
      {
          xdc.push_back(j);
          dcnames.push_back( string( pmp->SM[j],0, MAXDCNAME ));
      }
    }
...
void TProfil::ShowPhaseWindow( QWidget* par, const char *objName, int nLine )
{
    ...
    case 'P': // Phase : Ph_v__ or Ph_v2 or Phnam or Phnam2
        ...
        if( strncmp(objName, aObj[o_musf].GetKeywd(), MAXKEYWD)==0 ||
                strncmp(objName, aObj[o_musf2].GetKeywd(), MAXKEYWD)==0 )
        {    system = true;
             phname = string( rmults->GetMU()->SF[xph], 0, PH_RKLEN);
        }
        else  if( strncmp(objName, aObj[o_wd_sf].GetKeywd(), MAXKEYWD)==0 ||
                  strncmp(objName, aObj[o_wd_sf2].GetKeywd(), MAXKEYWD)==0 )
             {
