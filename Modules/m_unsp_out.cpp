...
void TUnSpace::to_text_file( fstream& ff, bool with_comments )
{
  ...
  // dynamic arrays - must follow static data
   ...
   prar.writeArrayS(  "SGp", usp->SGp[0], usp->nG, (long)MAXPHNAME);
    if( _comment )
   ...
   if( _comment )
    ...
    prar.writeArrayS(  "UnICn", usp->UnICn[0], UNSP_SIZE1, NAME_SIZE);
    if( _comment )
      ff << "\n## Names of columns for output array UgDC  ";
    prar.writeArrayS(  "UgDCn", usp->UgDCn[0], UNSP_SIZE1, NAME_SIZE);
    if( _comment )
      ff << "\n## Names of columns for output array  UaDc ";
    prar.writeArrayS(  "UaDCn", usp->UaDCn[0], UNSP_SIZE1, NAME_SIZE);
    if( _comment )
      ff << "\n## Names of columns for output array UnDCA ";
    prar.writeArrayS(  "UnDCAn", usp->UnICn[0], UNSP_SIZE2, NAME_SIZE);
    if( _comment )
      ff << "\n## List of insertain input parameters names ";
    prar.writeArrayS(  "ParNames", usp->ParNames[0], usp->nPG, (long)PARNAME_SIZE);
...
void TUnSpace::result_to_text_file( fstream& ff, bool with_comments )
{
  ...
  if( _comment )
   ...
   prar.writeArrayS(  "PhAlst", usp->PhAlst[0], usp->nPhA, 80 );
  ...
  {  if( _comment )
     ff << "\n# payoff matrix";
    prar.writeArray(  "POM", usp->POM, (short)(usp->Q*usp->Q), usp->Q );
  }
  if( usp->PvPOR == S_ON )
    {  if( _comment )
       ff << "\n# payoff matrix row";
