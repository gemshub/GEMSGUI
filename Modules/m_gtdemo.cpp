...
{
    return ( nrt >RT_ICOMP && nrt < (int)aMod.size());
}
///
...
//    pImp->MakeQuery();
    ...
    string prkey = string( gdp->prKey, 0, MAXRKEYLEN);
    int size[8];
    int nRT = RT_ICOMP;
    string script;
    if( gdp->expr )
    ...
    string xName = gdp->xNames;
    string yName = gdp->yNames;
...
AGAINRC:    //get  keypart
        ...
        //          goto AGAINRC;
        Nr = rt[gdp->nRT].GetKeyList( gdp->wcrk/*str.c_str()*/, aRklist );
        if( Nr<1 )
        {
...
TGtDemo::RecordPlot( const char* /*key*/ )
{
    ...
    else
    {
      ...
      for( ii=0; ii<gdp->dimXY[1]; ii++ )
          lnames.push_back( string(gdp->lNam0[ii+ndxy], 0, MAXGRNAME ));
      for( ii=0; ii<gdp->dimEF[1]; ii++ )
          lnames.push_back( string( gdp->lNamE[ii], 0, MAXGRNAME ));
      gd_gr = new GraphWindow( this, plt, gdp->name,
...
bool TGtDemo::SaveGraphData( GraphData *gr )
{
    int ii;
// We can only have one Plot dialog (modal one) so condition should be omitted!!
    ...
    if(  gdp->dimX > 1)
            ndxy =gdp->dimX;
    plot = (TPlotLine *)
           aObj[ o_gdplline].Alloc( gr->lines.size(), sizeof(TPlotLine));
