...
// select lists
    ...
    {  if( select_all )
         ...
         aList.push_back( string( mult->SB[ii], 0, MAXICNAME+MAXSYMB));
    }
    if( !select_all  )
    ...
    {  if( select_all )
       ...
       aList.push_back( string( mult->SM[ii], 0, MAXDCNAME));
    }
    if( !select_all  )
    ...
    {  if( select_all )
       ...
       aList.push_back( string( mult->SF[ii], 0, MAXPHNAME+MAXSYMB));
    }
    if( !select_all  )
 ...
 // setup default graphiks lines
  ...
  if( mtp->PsMode == RMT_MODE_S || mtp->PsMode == RMT_MODE_F || mtp->PsMode == RMT_MODE_B )
  {
     ...
     if( mtp->nPG > 0 && !(!*mtp->MGPid[0] || *mtp->MGPid[0] == ' '))
       {  phName = string(mtp->MGPid[0], 0, MAXSYMB);
            strip(phName);
       }
...
//            return; // system not changed  !!!! this is doubtful... DK 4.12.2011
    ...
    string skey;
    rt[RT_SYSEQ].MakeKey( RT_GEM2MT,  skey, RT_GEM2MT, 0, RT_GEM2MT,1,
...
//    rt[RT_SYSEQ].Find(mtp->sykey); // do we need this in RMT_MODE_S ?
    ...
    strncpy( mtp->sykey, skey.c_str(), EQ_RKLEN );
...
void TGEM2MT::make_A( long int siz_, char (*for_)[MAXFORMUNITDT] )
{
  ...
  for( ii=0; ii<siz_; ii++ )
  {
     ...
     form = string( for_[ii], 0, MAXFORMUNITDT );
     strip(form);
...
TGEM2MT::CalcPoint( int nPoint )
{
    if( mtp->PvMSg == S_OFF || nPoint >= mtp->nC )
     return;
    // push_back point to graph screen
    if( gd_gr )
...
TGEM2MT::RecordPlot( const char* /*key*/ )
{
    ...
    else
    {
      ...
      for( ii=0; ii<mtp->nYS; ii++ )
          lnames.push_back( string( mtp->lNam[ii], 0, MAXGRNAME ));
      for( ii=0; ii<mtp->nYE; ii++ )
          lnames.push_back( string( mtp->lNamE[ii], 0, MAXGRNAME ));
      gd_gr = new GraphWindow( this, plt, mtp->name,
...
// We can only have one Plot dialog (modal one) so condition should be omitted!!
    ...
    plot = (TPlotLine *) aObj[ o_mtplline].Alloc(
       gr->lines.size(), sizeof(TPlotLine));
    for( ii=0; ii<(int)gr->lines.size(); ii++ )
    {
        plot[ii] = gr->lines[ii];
