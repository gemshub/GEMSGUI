...
void TUnSpace::unsp_eqkey()
{
 ...
 //       K_IMM, usp->Pp, K_IMM, usp->TCp, RT_UNSPACE, 7, K_END );
    string uspstkey =usp->stkey;
    rt[RT_SYSEQ].MakeKey( RT_UNSPACE,  uspstkey, RT_UNSPACE, 0, RT_UNSPACE,1,
        RT_UNSPACE, 2, RT_UNSPACE, 3, K_IMM, usp->Bnamep,
        K_IMM, usp->Pp, K_IMM, usp->TCp, K_IMM, usp->timep, K_END );
    strncpy(usp->stkey, uspstkey.c_str(), EQ_RKLEN );
...
{
return usp->PhNum[ abs((int)usp->sv[j])-1];
}
...
// return: 1-not, 0-yes (propuskaet)
{
  ...
  if( usp->Pa_f_pha == S_ON )
     for(i=0; i<usp->N; i++)
      if(  usp->PhAndx[(abs((int)usp->sv[k])-1)*usp->N+i] != usp->f_PhA[i] )
         Filtr=1;
